    ...
    <div class="layui-form-item" style="margin:auto 20px auto 20px">
     ...
     <tbody id="goods-tbody-result">
     	<c:forEach items="${orList}" var="orList" varStatus="status">
     		<tr>
   			  <td>${orList.goodsId}</td>
		      <td>${orList.goods.goodsName}</td>
		      <td>${orList.goods.spec}</td>
		      <td>${orList.goods.brandName}</td>
		      <td>${orList.goods.uint}</td>
		      <td>${orList.goods.supplyPrice}</td>
		      <td>
...
function searchGoods(supplierid){
    ...
    $.ajax({  
        type: "post",       
        url: "<%=basePath%>godownEntry/iframeSearchListGoods",  
        data: {"goodsId": goodsId,"supplierid":supplierid}, 
