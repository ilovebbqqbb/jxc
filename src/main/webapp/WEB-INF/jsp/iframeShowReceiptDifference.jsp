...
<body>
 ...
 <legend>差异报告单详情</legend>
   ...
   <div style="width:200px;margin:10px auto auto auto">
  		<c:choose>
  		<c:when test="${differencePreview.differenceStatus == '已发货，请注意签收'}">
	  		<button class="layui-btn" onclick="iframeAjaxUpdate(${differencePreview.differenceId},3)">确认收货</button>
	  		<button class="layui-btn layui-btn-danger" onclick="iframeAjaxDelete(${differencePreview.differenceId})">删除</button>
  		</c:when>
  		<c:when test="${differencePreview.differenceStatus == '确认收货，订单已完成'}">
