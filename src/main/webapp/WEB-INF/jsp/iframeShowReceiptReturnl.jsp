...
<body>
 ...
 <legend>退货单详情</legend>
   ...
   <div style="width:200px;margin:10px auto auto auto">
  		<c:choose>
  		<c:when test="${returnPreview.returnStatus == '已发货，请注意签收'}">
	  		<button class="layui-btn" onclick="iframeAjaxUpdateReturn(${returnPreview.returnId},3)">确认收货</button>
			<button class="layui-btn layui-btn-danger" onclick="iframeAjaxDeleteReturn(${returnPreview.returnId})">删除</button>
  		</c:when>
  		<c:when test="${returnPreview.returnStatus == '确认收货，订单已完成'}">
