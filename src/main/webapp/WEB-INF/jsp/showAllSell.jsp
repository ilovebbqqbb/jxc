		  ...
		  <tbody>
		    <c:forEach items="${sellList}" var="sell">
		    	<tr>
		    		...
		    		<td>
		    		<c:choose>
		    		<c:when  test="${sell.sellStatus == '已下单'}">
		    		<a href="sendOut/${sell.sellId}" class="layui-btn">发货</a>
		    		</c:when>
		    		<c:otherwise>
		    		<button class="layui-btn layui-btn-disabled">已发货</button>
		    		</c:otherwise>
		    		</c:choose>
