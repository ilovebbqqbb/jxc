...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
		 	function createLoss(){
				window.open("<%=basePath%>lossOrder/toAddLoss","_self");
			}
			...
			function addLoss(orderId){
				 layui.use('layer', function(){
					  var layer = layui.layer;
				$.ajax({
	              	type : "POST",
	              	url : "<%=basePath%>lossOrder/addNewLoss",
	              	data : {"orderId":orderId},
	              	success : function(data) {
	              		if(data.resultMsg == "success") {
	              			layer.msg('订单成功报损，可发起退货',{time: 2000},function(){window.location.reload();});
	              		} else {
			...
			<c:forEach items="${oList }" var="o" >
				<tr>
					...
					<div class="layui-btn-group">
						    <button class="layui-btn" onclick="addLoss(${o.orderId})">审核通过</button>
				...
				<tr>
					<td><button class="layui-btn layui-btn-radius" onclick="createLoss()">添加报损单</button></td>
				</tr>
		</tbody>
	</table>
