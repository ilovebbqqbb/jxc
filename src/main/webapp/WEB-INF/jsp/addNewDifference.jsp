...
<body>
	<p style="text-align:center;margin-top: 20px"><font size="6">添加差异报告单</font></p>
<div style="margin:auto 20px auto 20px">
<div class="layui-form">
  <div class="layui-form-item">
    <label class="layui-form-label">订单编号：</label>
    <label id="differenceId" class="layui-form-label">${randomId}</label>
  </div>
</div>
<div class="layui-form">
  <div class="layui-form-item">
  ...
  <div class="layui-form-item">
    ...
    <div class="layui-input-block">
      <input id="differenceNum" type="text" name="differenceNum" value="0" readonly="readonly" autocomplete="off" class="layui-input">
    </div>
  </div>  
  <div class="layui-form-item">
    <label class="layui-form-label">商品金额</label>
    <div class="layui-input-block">
      <input id="differenceMoney" type="text" name="differenceMoney" value="0.0" readonly="readonly" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-form-item">
	        ...
	        $(this).find("td").each(function() {
	    	        if ($(this).index() == "6") {
	    	        	if($(this).children("input").val() == null || $(this).children("input").val() == "" || $(this).children("input").val() == "0"){
	    	        		$(this).children("input").focus();
	    	        		layer.tips('请输入差异数量', $(this).children("input"), {time: 3000});
	    	        		stop = true;
	    	        		return false;
	    	        	}
	    	        }
...
function addDifference() {
	var differenceId = $("#differenceId").text();
	var warehouseId = $("#warehouseId").val();
    var storeId = $("#storeId").val();
    var differenceNum = $("#differenceNum").val();
    var differenceMoney = $("#differenceMoney").val();
	...
	InsertDifferenceDetail();
    

    $.ajax({
    	type : "POST",
    	url : "<%=basePath%>sell/ajaxInsertNewDifference",
    	data : {"differenceId":differenceId,"warehouseId":warehouseId,"storeId":storeId,"differenceNum":differenceNum,"differenceMoney":differenceMoney,"operater":operater},
...
function InsertDifferenceDetail(){
	var differenceIdPath = $("#differenceId").text();
	var goodsIdPath = 0;
	var goodsNumPath = 0;
	var remarksPath = "";
	var tbNothing = "<tr><td colspan='10' style='text-align: center;'><i class='layui-icon' style='font-size: 20px; color: #FF0000;''>&#xe60b</i>请点击按钮添加商品</td></tr>";
	...
	if($('#tbody-result').html() != "" || $('#tbody-result').html() != null || $('#tbody-result').html() != tbNothing){
		$('#tbody-result tr').each(function() {
	        ...
	        ajaxInsertDifferenceDetail(differenceIdPath,goodsIdPath,goodsNumPath,remarksPath);
	    });
	}
	
}

function ajaxInsertDifferenceDetail(differenceId,goodsId,goodsNum,remarks){
	layui.use('layer', function(){
		  var layer = layui.layer;
	  ...
	  $.ajax({
	  	type : "POST",
	  	url : "<%=basePath%>sell/ajaxInsertDifferenceDetail",
	  	data : {"differenceId":differenceId,"goodsId":goodsId,"goodsNum":goodsNum,"remarks":remarks,"storeId":storeId},
	  	success : function(data) {
...
function tbTotal(){
	...
	if($('#tbody-result').html() != "" || $('#tbody-result').html() != null || $('#tbody-result').html() != tbNothing){
		...
		$("#differenceNum").val(tbTotalNumPath);
		$("#differenceMoney").val(tbTotalPath.toFixed(1));
	} else {
		$("#differenceNum").val(0);
		$("#differenceMoney").val(0.0);
	}
