...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
		 	function createReturn(){
				window.open("<%=basePath%>returnLoss/toAddReturnLoss","_self");
			}
			...
			function addReturn(orderId){
				...
				$.ajax({
	              	type : "POST",
	              	url : "<%=basePath%>returnLoss/ReviewReturn",
	              	data : {"orderId":orderId},
	              	success : function(data) {
	              		if(data.resultMsg == "success") {
	              			layer.msg('商品成功退货',{time: 2000},function(){window.location.reload();});
	              		} else {
...
<body>
	...
	<fieldset class="layui-elem-field layui-field-title"
		...
		<legend>退货单列表(${oList.size() })</legend>
	</fieldset>
	<table class="layui-table" lay-skin="line">
		...
		<tbody>
			<c:forEach items="${oList }" var="o" >
				<tr>
					...
					<td>
						<div class="layui-btn-group">
						    <button class="layui-btn" onclick="addReturn(${o.orderId})">审核通过</button>
				...
				<tr>
					<td><button class="layui-btn layui-btn-radius" onclick="createReturn()">添加退货单</button></td>
				</tr>
		</tbody>
	</table>
